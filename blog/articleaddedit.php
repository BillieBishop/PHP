...
//In case of post
if ($_GET['link'] == '1'){
    function getArticleAddForm($title = '', $body = '') {
...
ENDTAG;
    ...
    if (!isset($_POST['body'])) {
        ...
        if ($errorList) {
            ...
            echo getArticleAddForm($title, $body);
        } else {
            ...
            $sql = sprintf("INSERT INTO articles VALUES (NULL, '%s','%s', '%s', '%s', '%s')", mysqli_escape_string($conn, $_SESSION['user']['ID']), mysqli_escape_string($conn, date("Y-m-d")), mysqli_escape_string($conn, $title), mysqli_escape_string($conn, $body), mysqli_escape_string($conn, $target_file));
...
$sql = sprintf("SELECT * FROM articles, users WHERE authorID=users.ID", mysqli_escape_string($conn, $_GET['id']));
$result = mysqli_query($conn, $sql);
...
$imagePath = $row['imagePath'];
        function getArticleEditForm($title = '', $body = '') {
        $authorID = $_SESSION['user']['name'];
        $pubDate = date("Y-m-d");
        $f = <<< ENDTAG
